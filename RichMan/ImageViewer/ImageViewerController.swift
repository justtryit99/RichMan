...
public final class ImageViewerController: UIViewController {
    ...
    /* 
     客製化 
     20210202 增加callBackDelegate
     20210310 增加修改clioseButton樣式
    
    */
//    weak var callBackDelegate: CallBackDelegate?
    ...
    override public func viewDidLoad() {
        ...
        // 2021.08.31 修改size，過大時，動畫效果會跑版，有試過修改imageView.resize，但一樣跑版，後來修改源頭可正常
//        configuration?.imageView?.resizeImage()
        imageView.image = configuration?.imageView?.image ?? configuration?.image
        print("imageView.image?.size: \(String(describing: imageView.image?.size))")
        ...
        closeButton.setCorner(radius: 5)
    }
    
    func dismissVC() {
        dismiss(animated: true) { 
//            self.callBackDelegate?.doSomeThing(key: "dismissImageViewer")
        }
    }
}
